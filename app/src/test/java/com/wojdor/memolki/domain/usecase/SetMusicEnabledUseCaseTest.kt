...
class SetMusicEnabledUseCaseTest : AppTest() {

    private val settingsRepository = SettingsRepository(SettingsLocalDataSource(MockDataStore()))
    private lateinit var sut: SetMusicEnabledUseCase

    @Before
    override fun setup() {
        super.setup()
        sut = SetMusicEnabledUseCase(
            testDispatcher,
            settingsRepository
        )
    }
    ...
    @Test
    fun `when called then set music enabled`() = runTest {
        // when
        sut(false).test {
            // then
